package Chainsaw.ViT.TestUtils

import spinal.lib._
import spinal.core._
import spinal.core.sim._
import Chainsaw.ViT.TestUtils.UVM.DataCarrierUtils._
import Chainsaw.ViT.TestUtils.UVM.DataUtils._
import Chainsaw.ViT.TestUtils.UVM.OtherUtils.Scoreboard
import org.slf4j._
...
package object UVM {
  ...
  /** This package which contain some method for build UVM component
    ...
    */
  implicit class UVMComponentAgent[D <: Data](port: D) {
    ...
    /** This method which can be use to drive Data port by random value
      ...
      * @param driveWhen
      *   the drive enable signal, it should be function type parameter
      */
    def setRandomDriver(
      ...
      port match {
        case flow: Flow[_] =>
          FlowMasterDriver(clockDomain, flow.asInstanceOf[Flow[D]], latency, driveGap, randomMode, driveWhen) {
            payload =>
              payload.randomize()
              true
          }
        case stream: Stream[_] =>
          if (stream.valid.isInput) {
            StreamMasterDriver(clockDomain, stream.asInstanceOf[Stream[D]], latency, driveGap, randomMode, driveWhen) {
              payload =>
                payload.randomize()
                true
            }
    ...
    /** This function can be use to drive Data port by given stimulus
      ...
      *   the drive enable signal, it should be function type parameter
      * @param stimulus
      *   the stimulus to Data port for Simulation, if the port have more than one port, the stimulus should be a
      *   muti-Dimention ArraryBuffer, it will drive the ports in order
      * @tparam T
      *   the stimulus type
        ...
        case flow: Flow[_] =>
          FlowMasterDriver(clockDomain, flow.asInstanceOf[Flow[D]], latency, driveGap, randomMode, driveWhen) {
            payload =>
              ...
              require(
                payloads.size == driverCases.size,
                s"the testCase size is not match ! port number : ${payloads.size}\t testCases size : ${driverCases.size}"
              )
        ...
        case flow: Flow[_] =>
          FlowMonitor(clockDomain, flow.asInstanceOf[Flow[D]], latency, monitorGap, monitorWhen) { payload =>
            ...
            require(
              ...
              s"the monitor container size is not match ! port number : ${payloads.size}\t container size : ${results.size}"
            )
          ...
          StreamMonitor(clockDomain, stream.asInstanceOf[Stream[D]], latency, monitorGap, monitorWhen) { payload =>
            ...
            results.zip(payloads).foreach { case (result, baseType) =>
              baseType match {
                ...
                case enum: SpinalEnumCraft[_] => result += enum.toEnum.asInstanceOf[T]
                case _ =>
                  logger.warn(s"The setMonitor function is fail to monitor this stream port : ${stream.getName()}")
              }
            }
          }
        case data: Data =>
          ...
          FlowMonitor(clockDomain, flow.asInstanceOf[Flow[D]], latency, monitorGap = 1, monitorWhen = true) { payload =>
            ...
            results.zip(payloads).foreach { case (result, baseType) =>
              baseType match {
                ...
                case enum: SpinalEnumCraft[_] => result += enum.toEnum.asInstanceOf[T]
                case _ =>
                  logger.warn(s"The setMonitorAlways function is fail to monitor this flow port : ${flow.getName()}")
              }
            }
          }
        case stream: Stream[_] =>
          StreamMonitor(clockDomain, stream.asInstanceOf[Stream[D]], latency, monitorGap = 1, monitorWhen = true) {
            payload =>
              val payloads = payload.flatten
              require(
                payloads.size == results.size,
              ...
              results.zip(payloads).foreach { case (result, baseType) =>
                baseType match {
                  ...
                  case _ =>
                    logger
                      .warn(s"The setMonitorAlways function is fail to monitor this stream port : ${stream.getName()}")
                }
              }
          }
        case data: Data =>
          DataMonitor(clockDomain, data, latency, monitorGap = 1, monitorWhen = true) { payload =>
            ...
            results.zip(payloads).foreach { case (result, baseType) =>
              baseType match {
                ...
                case enum: SpinalEnumCraft[_] => result += enum.toEnum.asInstanceOf[T]
                case _ =>
                  logger.warn(s"The setMonitorAlways function is fail to monitor this data port : ${data.getName()}")
              }
            }
          }
    ...
    /** @param clockDomain
      ...
      */
    def setStreamTransactionAssert[H <: Data](clockDomain: ClockDomain): Unit = {
      require(
        port.isInstanceOf[Stream[H]],
        s"Only Stream type support transaction assert! You type is ${port.getClass.getSimpleName.init}"
      )
      StreamTransactionAssert(clockDomain, port.asInstanceOf[Stream[H]])
    }

    /** @param dut
      *   the output data of dut
      * @param ref
      *   the reference data
      * @tparam T
      *   the data container's data type
      ...
      * --- if the port is SpinalEnumCraft, it must be SpinalEnumElement[SpinalEnum]
      */
    def setScoreBoard[T](dut: ArrayBuffer[T], ref: ArrayBuffer[T]): Unit = {
      require(
        dut.length == ref.length,
        s"You input dut and ref size is misMatch, dut size: ${dut.length}, ref size: ${ref.length}!"
      )
      val scoreBoard = new Scoreboard[T]()
      dut.zip(ref).foreach { case (d, r) =>
        scoreBoard.pushDut(d)
