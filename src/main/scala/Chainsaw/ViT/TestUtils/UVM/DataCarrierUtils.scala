...
object DataCarrierUtils {
  ...
  object StreamMonitor {
    def apply[T <: Data](
        ...
        monitorGap: Int,
        monitorWhen: => Boolean
    )(callback: T => Unit): StreamMonitor[T] =
      new StreamMonitor(clockDomain, stream, latency, monitorGap, monitorWhen).addCallback(callback)
  }

  class StreamMonitor[T <: Data](
      clockDomain: ClockDomain,
      stream: Stream[T],
      latency: Int,
      monitorGap: Int,
  ...
  class StreamMasterDriver[T <: Data](
      clockDomain: ClockDomain,
      stream: Stream[T],
      latency: Int,
      driveGap: Int,
  ...
  object StreamSlaveDriver {
    def apply[T <: Data](
        ...
        randomMode: Boolean,
        driveWhen: => Boolean,
        activeLevel: Boolean = true
    ) = new StreamSlaveDriver(stream, clockDomain, latency, driveGap, randomMode, driveWhen, activeLevel)
  }

  class StreamSlaveDriver[T <: Data](
      stream: Stream[T],
      clockDomain: ClockDomain,
      latency: Int,
      driveGap: Int,
  ...
  object FlowMonitor {
    def apply[T <: Data](
        ...
        monitorGap: Int,
        monitorWhen: => Boolean
    )(callback: T => Unit): FlowMonitor[T] =
      new FlowMonitor(clockDomain, flow, latency, monitorGap, monitorWhen).addCallback(callback)
  }

  class FlowMonitor[T <: Data](
      clockDomain: ClockDomain,
      flow: Flow[T],
      latency: Int,
      monitorGap: Int,
