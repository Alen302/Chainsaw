...
object DataUtils {
  ...
  object DataDriver {
    def apply[D <: Data](
        ...
        randomMode: Boolean,
        driveWhen: => Boolean
    )(driver: D => Boolean) = new DataDriver(clockDomain, data, latency, driveGap, randomMode, driveWhen, driver)
  }

  class DataDriver[D <: Data](
      clockDomain: ClockDomain,
      data: D,
      latency: Int,
      driveGap: Int,
  ...
  object DataMonitor {
    def apply[D <: Data](clockDomain: ClockDomain, data: D, latency: Int, monitorGap: Int, monitorWhen: => Boolean)(
        callback: D => Unit
    ): DataMonitor[D] =
      new DataMonitor(clockDomain, data, latency, monitorGap, monitorWhen).addCallback(callback)
  }

  class DataMonitor[D <: Data](
      clockDomain: ClockDomain,
      data: D,
      latency: Int,
      monitorGap: Int,
