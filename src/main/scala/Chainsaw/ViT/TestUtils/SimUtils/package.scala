...
import spinal.core.sim.SpinalSimBackendSel.{VCS, VERILATOR}
import org.slf4j._
...
package object SimUtils {
  ...
  implicit class SpinalSimUtils(target: SpinalSimConfig) {
    ...
    def compileWithMake[T <: Component](rtl: => T): SimCompiled[T] = {
      ...
      target._waveFormat match {
        ...
        case _ =>
          logger.warn(
            s"This invoke is not generate valid Makefile file, Because the compileWithMake method of this type waveFormat : ${target._waveFormat.toString()} is not realize !"
          )
      }
  ...
  /** @param backend
    ...
    */
  def getSimConfig(
      backend: SpinalSimBackendSel = VERILATOR,
    ...
    (backend, workSpaceName, config) match {
      ...
      case (VERILATOR, workSpaceName, null) =>
        SimConfig.withFstWave.workspacePath(s"./$workSpacePath").workspaceName(workSpaceName).allOptimisation
      case (VERILATOR, null, config) =>
        SimConfig.withFstWave.withConfig(config).workspacePath(s"./$workSpacePath").allOptimisation
      case (VERILATOR, workSpaceName, config) =>
      ...
      case (VCS, null, null) =>
        SimConfig.withVCS(vcsFlags).withFSDBWave.workspacePath(s"./$workSpacePath").allOptimisation
      case (VCS, workSpaceName, null) =>
        SimConfig
          .withVCS(vcsFlags)
          .withFSDBWave
          .workspacePath(s"./$workSpacePath")
          .workspaceName(workSpaceName)
          .allOptimisation
      case (VCS, null, config) =>
        SimConfig.withVCS(vcsFlags).withFSDBWave.withConfig(config).workspacePath(s"./$workSpacePath").allOptimisation
      case (VCS, workSpaceName, config) =>
        SimConfig
          .withVCS(vcsFlags)
          .withFSDBWave
          .withConfig(config)
  ...
  implicit class DataSimUtils[D <: Data](port: D) {
    ...
    /** This function can be use to drive Data port by given initialValues without delay and hold given cycle
      ...
      * @param initialValues
      *   the stimulus to Data port for Simulation, if the data port have more than one port, the stimulus should be a
      *   muti-Dimention ArraryBuffer, it will drive the ports in order, the stimulus's size should be equal to port's
      *   size
      * @param holdCycles
          ...
          if (flow.valid.isInput) {
            ...
            flow.payload.flatten.zip(initCases).foreach { case (baseType, driver) =>
              baseType match {
                ...
                case _ =>
                  logger.warn(s"The setInitialValue function is fail to initiate this flow port : ${flow.getName()}")
              }
            }
            flow.valid #= false
          }
          ...
          if (stream.valid.isInput) {
            ...
            stream.payload.flatten.zip(initCases).foreach { case (baseType, driver) =>
              baseType match {
                ...
                case _ =>
                  logger
                    .warn(s"The setInitialValue function is fail to initiate this stream port : ${stream.getName()}")
              }
            }
            stream.valid #= false
          } else {
          ...
          data.flatten.zip(initCases).foreach { case (baseType, driver) =>
            baseType match {
              ...
              case enum: SpinalEnumCraft[_] =>
                enum #= driver.asInstanceOf[SpinalEnumElement[SpinalEnum]]
              case _ =>
                logger.warn(s"The setInitialValue function is fail to initiate this data port : ${data.getName()}")
            }
          }
