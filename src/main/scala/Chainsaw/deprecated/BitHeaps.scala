...
//
//import Chainsaw.arithmetic.{ArithInfo, compressorSolutionDir}
...
//      compressor.clbCost(width, considerCarry8, shouldPipeline) // divided by cost
//  }
//
...
//      lastBestCompressor: Compressor,
//      lastBestWidth: Int,
//      newBestCompressor: Compressor,
//      newBestWidth: Int,
...
//
//    Seq(lastBestWidth, newBestWidth).zip(Seq(lastBestCompressor, newBestCompressor).zipWithIndex).foreach { case (searchWidth, (compressor, i)) =>
//      val exactReductionEfficiency = candidateHeaps(i).getExactReductionEfficiency(compressor, considerCarry8 = true, searchWidth, 0, shouldPipeline)
//      firstStageCost(i) = compressor.clbCost(searchWidth, considerCarry8 = true, shouldPipeline)
//      firstStageReductionBits(i) = compressor
//        .inputFormat(searchWidth)
...
//        }
//        .map(_.length)
//        .sum - compressor.outputBitsCount(searchWidth)
//    }
//
//    val secondStageHeights = candidateHeaps.zip(Seq(lastBestWidth, newBestWidth)).map { case (heap, width) => heap.currentHeights.take(width) }
//
//    secondStageHeights.zip(Seq.fill(2)(Seq(lastBestCompressor, newBestCompressor)).zipWithIndex).foreach { case (heights, (candidateComp, i)) =>
...
//        val stageBestWidth      = ArrayBuffer.fill(2)(ArrayBuffer(lastBestWidth, lastBestWidth))
//
//        candidateComp.zipWithIndex.foreach { case (compressor, j) =>
//          var searchWidth = (heights.length - startColumnIndex) min compressor.widthMax
//          while (searchWidth >= compressor.widthMin) {
//            val exactReductionEfficiency = candidateHeaps(i).getExactReductionEfficiency(compressor, considerCarry8 = true, searchWidth, startColumnIndex, shouldPipeline)
...
//          secondStageCost(i) = bestCompressor.clbCost(bestWidth, considerCarry8 = true, shouldPipeline)
//          secondStageReductionBits(i) = bestCompressor
//            .inputFormat(bestWidth)
...
//            }
//            .map(_.length)
//            .sum - bestCompressor.outputBitsCount(bestWidth)
//        } else {
...
//    var bestWidth               = 1
//    var bestReductionEfficiency = bestCompressor.reductionEfficiency(bestWidth, considerCarry8, shouldPipeline)
//    var bestHeightReduction     = bestCompressor.heightReduction(bestWidth)
//
//    val candidates = compressors.tail
//      .sortBy(compressor => compressor.reductionEfficiency((currentWidth - columnIndex) min compressor.widthMax, considerCarry8, shouldPipeline))
//      .reverse
//
...
//          val exactReductionEfficiency = getExactReductionEfficiency(compressor, considerCarry8, width = -1, columnIndex, shouldPipeline)
//          val exactHeightReduction     = getExactHeightReduction(compressor, width = -1, columnIndex = columnIndex)
...
//            while (searchWidth >= compressor.widthMin) {
//              val exactReductionEfficiency = getExactReductionEfficiency(compressor, considerCarry8, width = searchWidth, columnIndex, shouldPipeline)
//              if (exactReductionEfficiency >= efficiencyBound) {
//                val isBetter = isBetterCompressorInCurrentHeap(compressor, searchWidth, bestCompressor, bestWidth, columnIndex, shouldPipeline, efficiencyBound, searchThreshold)
//                if (isBetter) {
...
////              if (exactReductionEfficiency >= (bestReductionEfficiency max efficiencyBound)) {
////                if (exactReductionEfficiency == bestReductionEfficiency) {
////                  val isBetter = compressor.outputBitsCount(searchWidth) > bestCompressor.outputBitsCount(bestWidth)
////                  if (isBetter) {
////                    if (bestReductionEfficiency > exactReductionEfficiency) logger.info(s"warning")
////                    bestReductionEfficiency = exactReductionEfficiency
////                    bestWidth               = searchWidth
////                    bestCompressor          = compressor
////                  }
...
//    val gpcs = compressors.tail
//      .filter(_.isFixed)
//      .sortBy(compressor => compressor.reductionEfficiency(width = -1, considerCarry8, shouldPipeline))
//      .reverse
//
//    val rowAdderEfficiencyInfo = compressors.tail
//      .filterNot(_.isFixed)
//      .flatMap { compressor =>
//        Range.inclusive((currentWidth - columnIndex) min compressor.widthMax, 1, -1).map { w =>
//          (compressor, w, getExactReductionEfficiency(compressor, considerCarry8, w, columnIndex, shouldPipeline))
//        }
//      }
...
//      maxEfficiencyRowAdderInfos.head._1,
//      maxEfficiencyRowAdderInfos.head._2,
//      maxEfficiencyRowAdderInfos.last._1,
//      maxEfficiencyRowAdderInfos.last._2,
//      columnIndex,
//      shouldPipeline,
//      efficiencyBound
//    )
...
//      if (finalStageBefore) tailStrategy(compressors, considerCarry8, tailBound, shouldPipeline, controlHeights)
//      else headStrategy(compressors, considerCarry8, headBound, shouldPipeline)
...
//      }
//      bitHeapConfigInfos += deprecated.BitHeapConfigInfo(table, positionLow, info.head.time)
//    }
//    BitHeaps(bitHeapConfigInfos: _*)
//  }
//
